...
public class Zoo {
   ...
   public static void main(String[] args) {
      ...
      Animal[] animals = {dog, cat, duck};   // 부모 타입인 Animal 배열에 서로 다른 자식 객체를 담을 수 있음
      for(Animal animal: animals) {             // "여러 종류의 동물을 부모 타입 하나로 통일해서 같은 방식으로 처리하겠다" (다형성)
         System.out.println(animal);
         animal.sound();
      }
      
      ((Duck)duck).layEggs();   // 캐스팅하기. (Duck)duck: "나 이거 Duck 맞아!!" 하고 형변환 하는 것
      ...
      for(int i = 0; i < animals.length; i++) {
         ...
         if(ani instanceof Duck) {   // ani의 타입은 Animal. Animal에는 layEggs()와 fly()가 없는데, 실제 객체가 Duck일 수도 있어서 확인. "혹시 너 Duck이야?"
            ((Duck)ani).layEggs();
            ((Fly)ani).fly();
         }
      }
      
      Fly fly = new Duck("","");   // "나는 오직 fly() 기능만 쓸 거야"
